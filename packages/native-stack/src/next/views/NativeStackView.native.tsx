...
function NativeStackViewContent({
  ...
  const { cards, sheets } = renderedRoutes.reduce<{
    ...
    (result, route) => {
      ...
      const isPopped = poppedByKey.has(route.key);
      const isSheet =
        SUPPORTS_FORM_SHEET && descriptor.options.presentation === 'formSheet';
      ...
      if (isSheet) {
        ...
        if (isPopped) {
          ...
          const isReplacingWithSheet =
            focusedRoute != null &&
            SUPPORTS_FORM_SHEET &&
            getDescriptor(focusedRoute).options.presentation === 'formSheet';

          if (isReplacingWithSheet) {
            throw new Error(
              `The form sheet route '${focusedRoute.name}' cannot replace '${route.name}' in the same native stack. Wait for the previous sheet to close before presenting another sheet.`
            );
          }
        }
        ...
        const previousDescriptor = isPopped
          ? poppedByKey.get(route.key)?.previousDescriptor
          : previousRoute == null
            ? undefined
            : getDescriptor(previousRoute);
